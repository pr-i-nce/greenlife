            ...
            {records && records.length > 0 ? (
              records.map((record, index) => (
                <tr key={record.id || index}>
                  <td data-label="Product Name">{displayValue(record.productName)}</td>
                  <td data-label="Price">{displayValue(record.cost)}</td>
                  <td data-label="Sold Unit(s)">{displayValue(record.quantity)}</td>
                  {/* <td data-label="Receipt number">{displayValue(record.receipno)}</td> */}
                </tr>
              ))
            ) : (
              <tr>
