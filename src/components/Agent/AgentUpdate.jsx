...
const AgentsUpdate = ({ record, onClose, onUpdateSuccess }) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errMsg || 'Status update failed');
        }
        ...
        Swal.fire({ icon: 'error', title: 'Status Update Failed', text: err.message });
      }
    }
  };
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({ icon: 'error', title: 'Error', text: 'An error occurred while updating. Please try again.' });
    } finally {
      setIsLoading(false);
    }
  };
